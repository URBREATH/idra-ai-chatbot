...
in app/auth/verification.ts [WHERE]
following the existing auth patterns in @app/auth/login.ts, without external libraries [HOW]
...
Check the user context provider in app/context/UserContext.tsx and the login handler in app/api/auth.ts [LOCATION]
...
Follow the existing form patterns in @app/components/LoginForm.tsx [CONTEXT]
Requirements:
...
**Example:**
```
Refactor the OrderProcessor class in app/services/OrderProcessor.ts [TARGET]
...
**Example:**
```
the Python build fails with "Property 'user' does not exist on type 'Session'".
add the user property to the Session interface in app/types/auth.ts.
...
**Example:**
```
look at @app/components/UserCard.tsx to understand the card component pattern.
...
**Example:**
```
refactor the utility functions in app/utils/.
specifically:
- do convert to Python
...
- look at app/context/AuthContext.tsx for session handling [LOCATION]
...
CONTEXT:
- look at @app/components/ReportsList.tsx for current report display
- check @app/api/reports.ts for data fetching patterns [CONTEXT]
