...
run tests after implementing. place in app/utils/validation.ts following existing util patterns.
```
...
**AFTER:**
```
write a parseDate function in app/utils/dates.ts that handles multiple formats. test cases:
- "2024-01-15" → Date object for Jan 15, 2024
- "01/15/2024" → Date object for Jan 15, 2024
- "January 15, 2024" → Date object for Jan 15, 2024
- "invalid" → null
- "" → null
follow the existing utility function patterns in @app/utils/. run tests after implementing.
```
...
**AFTER:**
```
users report that login fails after session timeout. check the auth flow in app/auth/, especially token refresh. write a failing test that reproduces the issue, then fix it. verify by running the auth test suite.
```
...
**AFTER:**
```
getting "Cannot read property 'user' of undefined" error on line 45 of app/components/Dashboard.tsx when loading the dashboard without being logged in. check the auth context provider and add proper null handling. write a test for the unauthenticated state, then fix it.
```
...
**AFTER:**
```
the app's memory usage grows over time when navigating between pages. I suspect event listeners aren't being cleaned up. check app/components/ for useEffect hooks that add listeners without cleanup. profile the heap in Chrome DevTools to identify the leak source. fix any missing cleanup functions and verify memory stays stable during navigation.
```
...
**AFTER:**
```
add a search feature to the products page following the existing search pattern in @app/components/UserSearch.tsx. requirements:
- debounce input by 300ms
- show loading spinner during search
- display "no results" when empty
- highlight matching text in results
add tests for debouncing and empty states. place component in app/components/ProductSearch.tsx.
```
...
**AFTER:**
```
add cursor-based pagination to the /api/products endpoint and ProductList component. follow the existing pagination pattern in @app/api/users.ts and @app/components/UserList.tsx. requirements:
- 20 items per page
...
1. read app/styles/ to understand current theming approach
2. add CSS variables for dark theme colors in app/styles/themes.css
3. create a ThemeContext in app/context/ThemeContext.tsx
4. add toggle switch to Settings.tsx
5. persist preference in localStorage
6. respect system preference as default
follow existing context patterns in @app/context/AuthContext.tsx. test the toggle persists across page reloads.
```
...
follow the existing form patterns in @app/components/ProfileForm.tsx. backend endpoint is POST /api/upload. add tests for file validation and error handling. verify by uploading various file types and sizes.
```
...
**AFTER:**
```
the UserDashboard component in @app/components/UserDashboard.tsx is 500+ lines. extract these sections into separate components:
...
**AFTER:**
```
app/api/users.ts and app/api/products.ts have duplicate error handling logic (lines 20-45 in each). extract a shared handleApiError function to app/api/utils.ts. requirements:
...
**AFTER:**
```
modernize app/legacy/OrderProcessor.js:
- convert from class to functional approach with hooks
- replace var with const/let
- convert promises to async/await
- add Python types (rename to .ts)
...
- proper spacing (use existing spacing variables from @app/styles/variables.css)
...
use existing CSS variables from @app/styles/. verify all states: empty, filled, error, disabled. test at 320px, 768px, and 1024px widths.
```
...
follow the existing animation patterns in @app/styles/animations.css. test with reduced motion enabled in browser settings.
```
...
**AFTER:**
```
audit the navigation component in @app/components/Nav.tsx for accessibility. check and fix:
...
**AFTER:**
```
add a GET /api/products/:id endpoint following the pattern in @app/api/users.ts. requirements:
...
**AFTER:**
```
read app/auth/ to understand current session handling, then add Google OAuth. implementation plan:
1. add Google OAuth credentials to .env.example
2. create callback handler in app/auth/google.ts
...
follow the event pattern in @app/events/. store registrations in the webhooks table (create migration). add tests for registration validation, signature verification, and retry logic.
```
...
2. update Product model in app/models/Product.ts
...
**AFTER:**
```
the getOrdersWithProducts query in app/repositories/OrderRepository.ts takes 3+ seconds for users with many orders. current query is on line 45. profile the query with EXPLAIN ANALYZE:
...
**AFTER:**
```
review the changes in @app/services/PaymentService.ts for:
- security issues (especially around handling card data)
- error handling completeness
- edge cases not covered
- consistency with existing service patterns in @app/services/
- test coverage gaps
...
**AFTER:**
```
audit the user input handling in @app/api/ for security vulnerabilities:
...
**AFTER:**
```
add input validation to the user registration endpoint in app/api/users.ts. validate:
...
return specific error messages for each validation failure. use the validation patterns in @app/utils/validators.ts. add tests for each validation rule including edge cases.
```
...
- when was app/payments/ first created?
...
**AFTER:**
```
resolve the merge conflict in app/components/Header.tsx between feature/new-nav and main. context:
...
PHASE 1 - Explore:
- read existing user-related code in app/users/ and app/components/
...
IMPLEMENTATION:
1. read current API structure in app/api/
2. create app/api/v1/ and app/api/v2/ directories
...
Follow the existing routing patterns in @app/router.ts.
```
...
**AFTER:**
```
create comprehensive tests for app/services/PaymentService.ts
