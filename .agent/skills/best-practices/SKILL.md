...
| **Describe symptoms** | "users report login fails after session timeout. check app/auth/, especially token refresh" |
...
AFTER:  "users report login fails after session timeout. check the auth flow
         in app/auth/, especially token refresh. write a failing test that
...
```
Phase 1: EXPLORE
"read app/auth/ and understand how we handle sessions and login.
...
```
BEFORE: "add OAuth"
AFTER:  "read app/auth/ and understand current session handling. create a plan
...
AFTER: "users report login fails after session timeout. check the auth flow
in app/auth/, especially token refresh. write a failing test that reproduces
...
chaining, add proper Python types. run the existing test suite after
...
AFTER: "read app/auth/ and explain how authentication works in this codebase.
...
| **Location** | "the code" | "auth module" | `app/auth/login.ts:42` |
...
| Check | Pass | Fail |
|-------|------|------|
| File locations | `app/auth/login.ts` | "the auth code" |
